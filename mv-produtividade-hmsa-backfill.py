"""
mv-produtividade-hmsa-backfill.py
==================================
Backfill de produtividade para HMSA / producao_ses_sp.
Processa todos os dias de novembro e dezembro de 2025
e janeiro de 2026.

Diferenças em relação ao backfill genérico:
  - Filtra usuarios_codigomv apenas por nm_unidade = 'HMSA'
  - Filtra o RDS apenas por nm_banco = 'producao_ses_sp'
  - Intervalo de datas fixo: 2025-11-01 → 2026-01-31
"""
...
DATAS: list[date] = (
    gerar_datas(date(2025, 11, 1), date(2025, 11, 30)) +
    gerar_datas(date(2025, 12, 1), date(2025, 12, 31)) +
    gerar_datas(date(2026, 1,  1), date(2026, 1,  31))
)
...
def buscar_prestadores() -> list:
    separador(f"PASSO 1 — Buscar prestadores HMSA no Supabase")
    ...
    logger.info(f"  {len(prestadores)} entradas em usuario_codigomv para unidade={NM_UNIDADE_ALVO}")
...
def consultar_rds(prestadores: list, data_iso: str) -> tuple:
    ...
    Filtra apenas nm_banco = producao_ses_sp e nm_unidade = HMSA.
    """
    ...
    for p in prestadores:
        try:
            cd = int(p["codigomv"])
            nm = p["nm_unidade"]   # sempre 'HMSA' aqui
    ...
    # Filtro fixo: apenas producao_ses_sp e HMSA
...
def main():
    separador("mv-produtividade-hmsa-backfill.py")
    logger.info(f"  Unidade  : {NM_UNIDADE_ALVO}")
    logger.info(f"  Banco RDS: {NM_BANCO_ALVO}")
    logger.info(f"  Total de datas: {len(DATAS)}  ({DATAS[0]} → {DATAS[-1]})")

    prestadores = buscar_prestadores()
    if not prestadores:
        logger.warning(f"Nenhum prestador encontrado para unidade={NM_UNIDADE_ALVO}. Encerrando.")
        return
    ...
    salvar_csv("hmsa_backfill_detalhe_nov2025_jan2026.csv", todos_detalhe)
    salvar_csv("hmsa_backfill_resumo_nov2025_jan2026.csv",  todos_resumo)
