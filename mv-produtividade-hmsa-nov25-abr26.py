"""
mv-produtividade-hmsa-nov25-abr26.py
======================================
Backfill de produtividade HMSA de novembro/2025 até abril/2026.

Diferenças em relação ao mv-produtividade-hmsa-backfill.py:
  - Intervalo: 2025-11-01 → 2026-04-30 (6 meses)
  - NÃO atualiza registros já existentes — apenas insere novos.
    Chave de deduplicação: (codigo_mv, data, nm_unidade)
  - Pré-carrega todos os registros existentes de uma vez para
    evitar N queries individuais ao Supabase.
"""
...
NM_BANCO_ALVO   = "producao_ses_sp"
NM_UNIDADE_ALVO = "HMSA"
...
# ============================================================
# INTERVALO DE DATAS — Nov/2025 → Abr/2026
# ============================================================

def gerar_datas(inicio: date, fim: date) -> list[date]:
    datas, d = [], inicio
    while d <= fim:
        datas.append(d)
        d += timedelta(days=1)
...
DATAS: list[date] = gerar_datas(date(2025, 11, 1), date(2026, 4, 30))

DATA_INICIO_STR = DATAS[0].isoformat()   # "2025-11-01"
DATA_FIM_STR    = DATAS[-1].isoformat()  # "2026-04-30"
...
def formatar_especialidade(esp) -> str:
    ...
    if isinstance(esp, str):
        ...
        if esp.startswith("[") and esp.endswith("]"):
            partes = [p.strip().strip("'\"") for p in esp[1:-1].split(",")]
            return ", ".join(p for p in partes if p)
    return esp or ""


# ============================================================
# PASSO 1 — Buscar prestadores HMSA no Supabase (uma vez só)
# ============================================================

def buscar_prestadores(sb: Client) -> list:
    separador("PASSO 1 — Buscar prestadores HMSA no Supabase")

    resp = (
        sb.table("usuario_codigomv")
        .select("usuario_id, codigomv, nm_unidade, usuarios(id, nome, especialidade)")
        .eq("nm_unidade", NM_UNIDADE_ALVO)
        .execute()
    )

    prestadores = resp.data or []
    logger.info(f"  {len(prestadores)} entradas para unidade={NM_UNIDADE_ALVO}")
    for p in prestadores:
        nome = (p.get("usuarios") or {}).get("nome", "?")
        logger.info(f"    {nome:<40} codigomv={p['codigomv']}")
    return prestadores


# ============================================================
# PASSO 2 — Pré-carregar registros já existentes no Supabase
#           Chave: (codigo_mv, data, nm_unidade)
# ============================================================

def carregar_existentes(sb: Client) -> set[tuple]:
    """
    Retorna set de (codigo_mv, data_str, nm_unidade) já presentes
    na tabela produtividade para HMSA no período do backfill.
    Paginado para contornar o limite de 1000 linhas do PostgREST.
    """
    separador("PASSO 2 — Carregar registros existentes (HMSA, Nov/25–Abr/26)")
    ...
    while True:
        resp = (
            ...
            .select("codigo_mv, data, nm_unidade")
            .eq("nm_unidade", NM_UNIDADE_ALVO)
        ...
        batch = resp.data or []
        for r in batch:
            existentes.add((str(r["codigo_mv"]), str(r["data"]), r["nm_unidade"]))
        if len(batch) < page_size:
            break
        offset += page_size

    logger.info(f"  {len(existentes)} registros já existentes no Supabase (serão pulados)")
    return existentes


# ============================================================
# PASSO 3 — Detectar colunas + Consultar RDS para uma data
# ============================================================
...
def detectar_colunas(cur) -> dict:
    ...
    logger.info(
        f"  prestador={mapa['prestador']}  tipo={mapa['tipo']}  "
...
def consultar_rds(prestadores: list, data_iso: str) -> tuple:
    ...
    cd_prestadores_set: set[int] = set()
    pares_validos: set[tuple]    = set()
    ignorados: list[str]         = []
    ...
    filtro_banco   = f"AND {col_b} = %s" if col_b else ""
    filtro_unidade = f"AND {col_u} = %s" if col_u else ""
    ...
    params = (
        ...
        + ([NM_BANCO_ALVO]    if col_b else [])
        + ([NM_UNIDADE_ALVO]  if col_u else [])
    )

    logger.info("  Executando query no RDS...")
...
def pivotar(rows: list, prestadores: list, pares_validos: set, data_iso: str) -> tuple:
    ...
    logger.info(f"  {len(detalhe)} linhas no detalhe, {len(registros_prod)} registros pivotados")
    return registros_prod, detalhe


# ============================================================
# PASSO 5 — Inserir apenas registros novos (sem atualizar)
# ============================================================

def inserir_novos(sb: Client, registros: list, existentes: set) -> tuple[int, int, int]:
    """
    Insere apenas os registros cuja chave (codigo_mv, data, nm_unidade)
    NÃO está em `existentes`. Registros já existentes são pulados.

    Retorna (inseridos, pulados, erros).
    """
    if not registros:
        logger.warning("  Nenhum registro pivotado para processar.")
        return 0, 0, 0

    inseridos = pulados = erros = 0

    for reg in registros:
        chave = (str(reg["codigo_mv"]), str(reg["data"]), reg["nm_unidade"])

        if chave in existentes:
            logger.debug(f"  [SKIP] {reg['nome']} / {reg['nm_unidade']} — {reg['data']} (já existe)")
            pulados += 1
            continue

        try:
            sb.table("produtividade").insert(reg).execute()
            existentes.add(chave)   # evita reprocessar na mesma execução
            logger.info(f"  [INS] {reg['nome']:<40} {reg['nm_unidade']} — {reg['data']}")
            inseridos += 1
        except Exception as e:
            logger.error(f"  [ERR] {reg.get('nome', '?')} / {reg.get('data', '?')}: {e}")
            erros += 1
...
def main():
    separador("mv-produtividade-hmsa-nov25-abr26.py")
    logger.info(f"  Unidade  : {NM_UNIDADE_ALVO}")
    logger.info(f"  Banco RDS: {NM_BANCO_ALVO}")
    logger.info(f"  Período  : {DATA_INICIO_STR} → {DATA_FIM_STR} ({len(DATAS)} dias)")
    ...
    # Pré-carrega existentes UMA vez para todo o período
    existentes = carregar_existentes(sb)

    todos_detalhe: list = []
    todos_resumo:  list = []
    total_inseridos = total_pulados = total_erros = 0
    ...
    for dia in DATAS:
        ...
        separador(f"Inserção — {data_br}")
        ins, pul, err = inserir_novos(sb, registros_prod, existentes)
        total_inseridos += ins
        total_pulados   += pul
        total_erros     += err
        logger.info(f"  Dia {data_br}: {ins} inseridos, {pul} pulados, {err} erros")

    # CSVs consolidados
    salvar_csv("hmsa_backfill_detalhe_nov25_abr26.csv", todos_detalhe)
    salvar_csv("hmsa_backfill_resumo_nov25_abr26.csv",  todos_resumo)

    separador("CONCLUÍDO")
    logger.info(f"  Período  : {DATA_INICIO_STR} → {DATA_FIM_STR}")
    logger.info(f"  Inseridos: {total_inseridos}")
    logger.info(f"  Pulados  : {total_pulados}  (já existiam — não foram alterados)")
